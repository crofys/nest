 ...
 * @LastEditTime: 2019-11-29 16:15:42
 * @LastEditors: 徐长剑
 */
...
export const Curd = (options: ICurd): ClassDecorator => {
  return (target: any) => {
    ...
    controllerList.map(key => {
      // const { name } = key;
      // if (key !== 'constructor') {
      // target.prototype[key] = curdController[key];
      // target.prototype[key] = (...args) => {
      //   return curdController[key].apply(this, args);
      // };
      console.log(curdController[key].bind(CurdController), '=======');
      Reflect.defineProperty(target.prototype, key, {
        value: curdController[key].apply(CurdController),
        // value: () => {
