 ...
 * @LastEditTime: 2019-11-28 14:30:47
 * @LastEditors: 徐长剑
 */
...
export const Curd = (options: ICurd): ClassDecorator => {
  return (target: any) => {
    // return class
    const { model } = options;
    const curdService = new CurdService(model);
    console.log(Reflect.getPrototypeOf(curdService.__proto__), '===='); // 'A'
    console.log(Reflect.getMetadata('classMetaData', target), 'Reflect.getMetadata'); // 'A'
